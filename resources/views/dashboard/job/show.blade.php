...
@section('content')
    <div class="container mt-5 mx-auto overflow-hidden">

        <h1 class="text-3xl dark:text-white font-medium text-center">{{ $organisation->label }}</h1>
        <div class="flex justify-around items-center container">
            <div class="border border-red-500 rounded-xl py-8 px-10 dark:bg-gray-800 bg-gray-300 dark:text-white ">
                Argent d'entreprise: {{ number_format($organisation->getTreasory()) }} $
            </div>
            <div class="border border-red-500 rounded-xl py-8 px-10 dark:bg-gray-800 bg-gray-300 dark:text-white">
                Nombre d'employés: {{ $organisation->members->count() }}
            </div>

          
        </div>
        <div class="container grid px-6 mx-auto  overflow-hidden">
                    ...
                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr
                                ...
                                <th class="px-4 py-3">Grade Entreprise</th>
                            </tr>
                        </thead>
                            ...
                            @foreach ($organisation->members as $member)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3 text-sm">
                                        <a
                                        href="{{ route('dashboard-player.show', ['player' => $member]) }}">{{ $member->name }}</a>
                                    </td>

                                    <td class="px-4 py-3 text-sm">
                                        {{ number_format($member->get_Money() + $member->get_Bank(),2) }} $
                                    </td>

                                    <td class="px-4 py-3">
                                        {{ $member->getJobGrade()->label }}
                                    </td>
                                </tr>
                            @endforeach
            ...
            <div class="mt-10">
                <div class="card-header">
                    <h3 class="dark:text-white">Treasories</h3>
                </div>
                        ...
                        <tbody class=" divide-y dark:divide-gray-700 dark:bg-gray-800">
                            @foreach ($organisation->vehicules as $vehicule)
                                <tr class="text-gray-700 dark:text-gray-400">
                                    ...
                                    <td class="px-4 py-3">
                                        {{ $vehicule->player && $vehicule->owner != '1'? $vehicule->player->name . ' (' . ($organisation->isMember($vehicule->player) ? 'employé' : 'non employé') . ')': 'Aucune attribution' }}
                                    </td>
                                    <td class="px-4 py-3">
        ...
        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 transform translate-y-1/2" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0  transform translate-y-1/2" @click.away="closeModal" @keydown.escape="closeModal" class="w-full px-6 py-4 overflow-hidden  rounded-t-lg dark:bg-gray-800 sm:rounded-lg sm:m-4 sm:max-w-xl" role="dialog" id="modal" style="display: none;">
          ...
          <div class="mt-4 mb-6">
            ...
            <p class="text-sm text-gray-700 dark:text-gray-400">
                Etes-vous sur de vouloir supprimer la vehicule de ce joueur ?
